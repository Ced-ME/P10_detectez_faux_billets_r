...
```
On remarque avec l'ébouli qu'il est nécessaire d'avoir 5 composantes pour expliquer 90% de l'inertie des 6 variables.
Il n'est pas très utile de réduire la dimensionalité dans notre si nous perdons en précision pour expliquer la variance.
...
La répartition des "False" et "True" de chaque cluster nous indique que :
- Le cluster 1 correspond aux faux billets avec 96.7 % de faux billets
- Le cluster 2 correspond aux vrais billets avec 98.8 % de vrais billets
...
```
Ici nous avons :
- cluster 1 avec 98.1 % de faux billets
- cluster 2 avec 97.7 % de vrais billets
- cluster 3 avec 99 % de vrais billets
pour une efficacité globale de 98.3 %
...
```
Critère AIC initial 97.22
Après avoir retiré "diagonal", le modèle à le meilleur AIC avec 95.23.
Les autres étapes de suppression de variable augmentent l'AIC ce qui indique qu'elles sont importantes pour le modèle.
...
1 correspondant à la valeur "vrai" de is_genuine, un coefficient positif indique que si la valeur augmente, la probabilité de "vrai" billet augmente également
inversement un coefficient négatif indique que plus la valeur de la variable augmente plus la probabilité d'un "faux" billet augmente également.
...
predic_rl_model_1 <- predict(rl_model_1, type = "response")

# Convertir les probabilités en classes (0 ou 1) en utilisant un seuil de 0.5
rl_model_1_classes <- ifelse(predic_rl_model_1 > 0.5, "True", "False")

# Créer la matrice de confusion
confusion_rl_model_1 <- table(Predicted = rl_model_1_classes, Actual = data$is_genuine)
print(confusion_rl_model_1)
...
Pour une probabilité supérieur à 0.5, le billet est prédit comme bon et en dessous de 0.5 le billet est prédit comme faux.
La matrice de confusion montre que le modèle a une très bonne performance en classant les cas True et False correctement avec peu d'erreurs avec un seuil à 0.5.
...
precision_rl_model_1 <- confusion_rl_model_1[2, 2] / (confusion_rl_model_1[2, 2] + confusion_rl_model_1[2, 1])
recall_rl_model_1 <- confusion_rl_model_1[2, 2] / (confusion_rl_model_1[2, 2] + confusion_rl_model_1[1, 2])
specificity_rl_model_1 <- confusion_rl_model_1[1, 1] / (confusion_rl_model_1[1, 1] + confusion_rl_model_1[2, 1])
f1_score_rl_model_1 <- 2 * (precision_rl_model_1 * recall_rl_model_1) / (precision_rl_model_1 + recall_rl_model_1)
# Calculer Accuracy
accuracy_rl_model_1 <- (confusion_rl_model_1[1, 1] + confusion_rl_model_1[2, 2]) / sum(confusion_rl_model_1)      # (TP + TN) / Total
...
roc_curve <- roc(data$is_genuine, predic_rl_model_1)
...
```{r}
head(predic_rl_model_1)
```

#### a- Ajustement du seuil pour réduire le taux de faux positifs

La mesure de performance concernant la détection des faux billets est la spécificité dont le calcul est le suivant : VN/(VN+FP)
Cela nous donne la proportion de vrais négatifs détectés parmi les négatifs.
...
specificities <- sapply(thresholds, function(t) {
  calculate_specificity(predic_rl_model_1, data$is_genuine, t)
})
```
...
```{r}
final_predictions <- ifelse(predic_rl_model_1 > best_threshold, "True", "False")
...
# Calculer Accuracy
accuracy_2 <- (final_confusion_matrix[1, 1] + final_confusion_matrix[2, 2]) / sum(final_confusion_matrix)      # (TP + TN) / Total
# Afficher les résultats
...
```{r}
print(paste("L'éfficacité golobale (accuracy) du modèle avec un seuil de 0.5 était de", round(accuracy_rl_model_1*100, 2), "% tandis que pour le second avec le seuil qui réduit au maximum le risque de faux positif, l'éfficacité globale est de", round(accuracy_2*100, 2), "%"))
```
...
# Évaluer le modèle
confusionMatrix(rf_predictions, testTarget)
...
library(caret)
confusionMatrix(predictions_2, testData_selected$is_genuine, positive = "1")

...
best_threshold
```
Le Seuil optimal offrant la meilleur spécificité, c'est à dire le moins de risque de faux positifs était 
0.97 dans notre modèle.
...
resultat <- data.frame(id = ids, is_genuine = predictions, proba = round(predicted_prob*100,2))
...
resultat_rn <- data.frame(id = ids, is_genuine = classe, proba = round(rn_pred*100,2))
...
On observe dans ce tableau récapitulatif que les prédictions sont identiques pour is_genuine c'est à dire pour la détection de faux ou vrai pour ce jeu de données.  

Cependant on observe également une différence de certitude entre les deux modèles :  
- Le modèle de régression semble avoir plus de certitude avec des probabilités proches de 0 pour les faux, et proches de 100 pour les vrais.  
- Le modèle de réseau neuronal semble lui plus nuancer, B_1 est moins proche de 0 et B_5 a 76.55% d'être vrai.  
